...
export const updateApplicationStatus = createAsyncThunk(
  ...
  async ({ id, newStatus }, { rejectWithValue }) => {
    try {
      const { data } = await axios.put(`/api/v1/application/${id}/status`, {
        status: newStatus,
      });
      return data;
    } catch (error) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateApplicationStatus.fulfilled, (state, action) => {
        state.loading = false;
        state.message = "Application status updated successfully";
        state.applications = state.applications.map((app) =>
          app._id === action.payload.application._id
...
export const fetchEmployerApplications = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `https://online-job-portal-o1cv.onrender.com/api/v1/application/employer/getall`,
      {
...
export const fetchJobSeekerApplications = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `https://online-job-portal-o1cv.onrender.com/api/v1/application/jobseeker/getall`,
      {
        withCredentials: true,
      }
    );
...
export const postApplication = (data, jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `https://online-job-portal-o1cv.onrender.com/api/v1/application/post/${jobId}`,
      data,
      {
        withCredentials: true,
...
export const deleteApplication = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://online-job-portal-o1cv.onrender.com/api/v1/application/delete/${id}`,
      { withCredentials: true }
    );
    dispatch(
